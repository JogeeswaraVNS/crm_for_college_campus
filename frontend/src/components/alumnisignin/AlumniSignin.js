...
import Alumnidashnav from './Alumnidashnav';

class Alumni extends React.Component {
    constructor(props) {
        super(props);
        this.state = {AlumniID:'',Password:'',check:'',showpassword:false};
      ...
      componentDidMount() {
       ...
       checkLogin = () => {
           const Login=localStorage.getItem('AlumniID')
           console.log(Login)
           if (Login!=null){
               this.setState({AlumniID:Login,check:200})
            }
            else{
                this.setState({check:''})
            }
        ...
        handleSubmit (event) {
            axios.post('http://127.0.0.1:5000/alumni/login',this.state)
    ...
    render() {
      return (
        <div style={{marginBottom:'-3rem'}}>
            ...
            <div>
                {localStorage.setItem('AlumniID',this.state.AlumniID)}
                {/* <h1>Welcome {this.state.AlumniID}</h1> */}
                <Alumnidashnav AlumniID={this.state.AlumniID}/>
            </div>
            :            
            <div className="alumnisignin-photo">
                <h3 style={{marginTop:'-3rem',marginBottom:'2.5rem'}}
                className={this.state.check===401?"text-danger":"text-white"}
                >Use Valid Alumni ID and Password To Login</h3>
            <div className="form-container">
                <div className="img-holder"></div>
                {/* <form onSubmit={SubmitForm}> */}
                <form onSubmit={this.handleSubmit} autoComplete='off'> 
                <h3 className="text-center text-white mb-4"><strong>Alumni </strong>account</h3>
                    ...
                    <label htmlFor="check" className="p-1 mt-3 text-white"><h5>Show Password</h5></label>
