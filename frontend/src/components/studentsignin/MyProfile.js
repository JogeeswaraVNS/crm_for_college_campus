...
class MyProfile extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {Rollno:this.props.Rollno,Res:'',Data:[],Name:'',OldPassword:'',NewPassword:'',Confirm:'',Show:false,showpassword:false};
    ...
    //   axios.get(`http://127.0.0.1:5000/get/${this.props.Rollno}/`).then(r=>this.setState({Department:r.data})).catch(err=>console.log(err))
      axios.get(`http://127.0.0.1:5000/get/students/details/${this.props.Rollno}/`).then(r=>this.setState({Data:r.data})).catch(err=>console.log(err))
     };
    ...
    changePassword(event){
        ...
        if(this.state.NewPassword===this.state.Confirm){
            ...
            axios.put('http://127.0.0.1:5000/student/update',this.state)
  ...
  return (
    ...
    <div>
            ...
            <div>
<h4 className='mt-2'>Welcome to Your Profile, {this.state.Rollno}</h4>
...
<div style={{width:'70%',display:'block',margin:'auto'}} className='mt-4'>
      <div style={{backgroundColor:'#82001a'}} key={index} className="card text-white px-4 pt-3 mt-3">
        <h3 className="mb-3">{d.name}</h3>
        <p className="">
          <strong>Roll No :</strong> {d.rollno} | <strong>Department :</strong> {d.department} | <strong>City :</strong> {d.city}
        </p>

        <p className="">
          <strong>Gmail :</strong> {d.gmail} | <strong>Phone No :</strong> {d.phone_no}
        </p>
      </div>
</div>
    ))}
