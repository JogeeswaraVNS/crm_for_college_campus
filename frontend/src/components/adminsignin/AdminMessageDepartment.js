...
function AdminMessageDepartment(props) {
  ...
  const handleChange = (e) => {
    ...
    if (checked) {
      setUserInfo({
        ...
        response: [...departments, value],
      });
    }
  
    else {
      setUserInfo({
        departments: departments.filter((e) => e !== value),
        response: departments.filter((e) => e !== value),
      });
    }
  };
  ...
  useEffect(()=>{
    ...
    axios.get(`http://127.0.0.1:5000/get/departments`)
    .then(r=>setDepartmentData(r.data))
  ...
  function AdminDepthandleSubmit(event){
    ...
    axios.post(`http://127.0.0.1:5000/admin/${props.AdminID}/message/department/`,{Response:userinfo.response,
        ...
        <h4 style={{display:'block',margin:'auto',backgroundColor:'#82001a',borderRadius:''}} className='py-3 text-white my-4'>
          ...
          <div className="form-floating mt-3 mb-3 text-center">
              ...
              <textarea
                ...
                name="response"
                value={userinfo.response.length === 0 ? "Select the Department(s) to post a message to Selected Department(s)" : userinfo.response}
                placeholder="Posting Message to following Departments"
...
<div className='d-flex my-3' style={{justifyContent:'space-evenly'}}>
          <div>
  <h5>Selected : {userinfo.response.length}</h5>
</div>
<div>
  <h5>Not Selected : {13-userinfo.response.length}</h5>
</div>
</div>
        </div>
      </div>
...
export default AdminMessageDepartment